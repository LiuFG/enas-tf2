...
class MicroChild(Model):
  ...
  def _apply_drop_path(self, x, layer_id):
    ...
    layer_ratio = float(layer_id + 1) / (self.num_layers + 2)  # layer增加，drop概率增大
    ...
    drop_path_keep_prob = 1.0 - step_ratio * (1.0 - drop_path_keep_prob)  # step增加，drop概率增大
  ...
  def _enas_layer(self, layer_id, prev_layers, arc, out_filters):
    ...
    layers = [prev_layers[0], prev_layers[1]]
    layers = self._maybe_calibrate_size(layers, out_filters, is_training=True)  # 保证HW相同，C为out_filter
    used = []
    for cell_id in range(self.num_cells):
    ...
    out = tf.gather(out, indices, axis=0)   #输出是没有用过的 即used=0
